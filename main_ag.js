...
// v6.09.0 Multi-Slot Save Architecture
window.activeSaveSlot = 1;
window.getSaveSlotInfo = function(slot) {
    try {
        let saved = localStorage.getItem('ag_save_slot_' + slot);
        if (!saved) return `Slot ${slot}: Empty`;
        let state = JSON.parse(saved);
        let cName = state.courseName || "Unknown Course";
        let h = state.hole || 1;
        let pc = state.activePlayerCount || 1;
        return `Slot ${slot}: ${cName}, Hole ${h}, ${pc} Player${pc > 1 ? 's' : ''}`;
    } catch(e) { return `Slot ${slot}: Corrupted Data`; }
};
...
window.takeAITurn = function(isSim = false) {
    ...
    // v6.23.0 Bill the Legend Club Name Override
    let displayClub = (p.name === "Bill the Legend") ? "Ralph" : club.name;
    let setupMsg = isPutting ? `${p.name} is reading the green...` : `${p.name} equips ${displayClub} and lines up the shot...`;
...
window.saveGame = function(slot) {
    // v6.08.0 Sandbox Isolation & Sync
    if (typeof gameMode !== 'undefined' && gameMode !== 'course') return;
    if (typeof window.saveActivePlayer === 'function') window.saveActivePlayer();
    let targetSlot = slot || window.activeSaveSlot || 1;
    window.activeSaveSlot = targetSlot;
    const state = {
    ...
    try { localStorage.setItem('ag_save_slot_' + targetSlot, JSON.stringify(state)); } catch(e) {}
};

window.loadGame = function(slot) {
    try {
        let targetSlot = slot || window.activeSaveSlot || 1;
        const saved = localStorage.getItem('ag_save_slot_' + targetSlot);
        ...
        // v6.08.0 State Sync & Audio Bleed Fix
...
window.initGame = function() {
    // v6.09.0 Legacy Save Migration
    let legacySave = localStorage.getItem('ag_save_state');
    if (legacySave) {
        localStorage.setItem('ag_save_slot_1', legacySave);
        localStorage.removeItem('ag_save_state');
    }
    window.initAudio();
    
    // v6.09.1 Auto-load removed — use Clubhouse > Resume Session to load a save slot
...
window.generateTelemetryDump = function() {
    ...
    dump += `**Engine Version:** ${window.AG_VERSION || "v6.05.0"}\n`;
    dump += `**Course:** ${window.currentCourse ? window.currentCourse.name : "Unknown"}\n`;
    dump += `**Focus:** ${typeof focusModes !== 'undefined' && focusModes[focusIndex] ? focusModes[focusIndex].name : "Standard"}\n\n`;
    dump += `# SHOT LOG\n`;
    if (typeof holeTelemetry !== 'undefined') {
        dump += holeTelemetry.join('\n\n');
    } else {
        dump += "No telemetry recorded.";
    }
    return dump;
};
